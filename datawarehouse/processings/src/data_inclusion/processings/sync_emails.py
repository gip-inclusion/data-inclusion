...
def sync_emails(emails: list[str]) -> list[dict[str, Any]]:
    BREVO_API_KEY = os.getenv("BREVO_API_KEY")
    if not BREVO_API_KEY:
        print("BREVO_API_KEY is not set, skipping")
        return []

    BREVO_CURRENT_CONTACTS_LIST_ID = os.getenv("BREVO_CURRENT_CONTACTS_LIST_ID")
    BREVO_ALL_CONTACTS_LIST_ID = os.getenv("BREVO_ALL_CONTACTS_LIST_ID")

    brevo_client = BrevoClient(token=BREVO_API_KEY)
    contacts = list(brevo_client.list_contacts(BREVO_ALL_CONTACTS_LIST_ID))
    ...
    # Those are kept because in case there is a mishap, we can track what happened.
    print(f"{BREVO_ALL_CONTACTS_LIST_ID=}")
    print(f"{BREVO_CURRENT_CONTACTS_LIST_ID=}")
    if BREVO_CURRENT_CONTACTS_LIST_ID:  # avoid updating the lists by mistake
        brevo_client.import_to_list(BREVO_ALL_CONTACTS_LIST_ID, emails)
        brevo_client.import_to_list(BREVO_CURRENT_CONTACTS_LIST_ID, emails)
