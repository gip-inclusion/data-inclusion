...
def get_thematiques_by_group():
    ...
    for thematique in schemas.Thematique:
        try:
...
def get_sub_thematiques(thematiques: list[schemas.Thematique]) -> list[str]:
    """
...
def filter_services_by_thematiques(
    query: sqla.Select,
    thematiques: list[schemas.Thematique],
):
    return query.filter(
...
def filter_services_by_modes_accueil(
    query: sqla.Select,
    modes_accueil: list[schemas.ModeAccueil],
):
    filter_stmt = """\
    EXISTS(
        SELECT
...
def filter_services_by_types(
    query: sqla.Select,
    types: list[schemas.TypologieService],
):
    filter_stmt = """\
    EXISTS(
        SELECT
    ...
    if commune_instance is not None:
        ...
        query = query.filter(
            sqla.or_(
                ...
                == schemas.ZoneDiffusionType.PAYS.value,
                sqla.and_(
                    models.Service.zone_diffusion_type
                    == schemas.ZoneDiffusionType.COMMUNE.value,
                    models.Service.zone_diffusion_code == commune_instance.code,
                ),
                sqla.and_(
                    models.Service.zone_diffusion_type
                    == schemas.ZoneDiffusionType.EPCI.value,
                ...
                sqla.and_(
                    models.Service.zone_diffusion_type
                    == schemas.ZoneDiffusionType.DEPARTEMENT.value,
                    models.Service.zone_diffusion_code == commune_instance.departement,
                ),
                sqla.and_(
                    models.Service.zone_diffusion_type
                    == schemas.ZoneDiffusionType.REGION.value,
        ...
        query = query.filter(
            sqla.or_(
                ...
                models.Service.modes_accueil.contains(
                    sqla.literal([schemas.ModeAccueil.A_DISTANCE.value])
                ),
            )
        )
        ...
        query = query.add_columns(
            (
                sqla.case(
                    (
                        models.Service.modes_accueil.contains(
                            sqla.literal([schemas.ModeAccueil.EN_PRESENTIEL.value])
                        ),
                        (
