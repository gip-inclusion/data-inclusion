...
def test_duplicates_are_correctly_found(pytestconfig):
    ...
    raw = _read_csv("tests/unit/dedupe_inputs.csv")
    ...
    if should_update_duplicates:
        json.dump(output_data, open("tests/unit/dedupe_outputs.json", "w"), indent=4)

    expected_data = json.load(open("tests/unit/dedupe_outputs.json"))
