...
VELO_THEMATIQUE = "mobilite--mobilite-douce-partagee-collective"


@dataclass
class FranceTravailMarts:
    ...
    def run_with_data(
        ...
        print("Running France Travail marts with enriched data...")
        self.db.create_schema(self.target_schema)

        adresses = self.db.read_table(
            self.intermediate_schema, "int_france_travail__adresses_v1"
        )
    ...
    def _build_services(
        ...
        for row in enriched.to_dicts():
            ...
            thematiques = parse_json_array(row.get("thematiques")) or []
            if nom and "velo" in nom.lower():
                thematiques = thematiques + [VELO_THEMATIQUE]
            ...
            out = {
                ...
                **{
                ...
                "nom": nom,
                "description": format_description(row.get("description")),
                "thematiques": thematiques if thematiques else None,
