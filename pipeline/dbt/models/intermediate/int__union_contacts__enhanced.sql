...
rgpd_notices AS (
    ...
    WHERE SPLIT_PART(contact_uid, ':', 2) = 'services'
),
...
final AS (
    SELECT
        ...
        -- use contact details if there is no rgpd notice or if the notice was not objected to
        CASE WHEN rgpd_notices.id IS NULL OR NOT rgpd_notices.was_objected_to THEN contacts.contact_nom_prenom END AS "contact_nom_prenom",
        CASE WHEN rgpd_notices.id IS NULL OR NOT rgpd_notices.was_objected_to THEN contacts.courriel END           AS "courriel",
        CASE WHEN rgpd_notices.id IS NULL OR NOT rgpd_notices.was_objected_to THEN contacts.telephone END          AS "telephone",
