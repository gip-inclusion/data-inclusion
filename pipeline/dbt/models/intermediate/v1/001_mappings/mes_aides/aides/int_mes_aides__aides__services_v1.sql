...
publics AS (
    SELECT
        ...
        NULLIF(
            ARRAY_REMOVE(
                ARRAY_AGG(
                    DISTINCT
                    CASE
                        WHEN conditions.value ~* 'tou. public'
                            THEN 'tous-publics'
                        WHEN conditions.value ~* 'inscrit france travail|insertion professionnelle|en recherche d''emploi|etre inscrit en mission locale'
                            THEN 'demandeurs-emploi'
                        WHEN conditions.value ~* 'allocataire rsa|beneficiaire des minimas sociaux'
                            THEN 'beneficiaires-des-minimas-sociaux'
                        WHEN conditions.value ~* 'en situation de handicap'
                            THEN 'personnes-en-situation-de-handicap'
                        WHEN conditions.value ~* 'entre \d{2} et [1-3]\d ans|moins de [2-3]\d ans|de \d{2} a [1-3]\d ans'
                            THEN 'jeunes'
                        WHEN conditions.value ~* 'plus de [5-6]\d ans'
                            THEN 'seniors'
                        WHEN conditions.value ~* 'etudiant|stagiaire|lyceen|apprenti|alternant|cfa'
                            THEN 'etudiants'
                        WHEN conditions.value ~* 'interimaire|saisonnier|alternant'
                            THEN 'actifs'
                    END
        ...
        ARRAY_TO_STRING(ARRAY_AGG(conditions.value), ', ') AS "publics_precisions"
    FROM {{ ref('stg_mes_aides__aides__conditions') }} AS conditions
    GROUP BY conditions.aide_id
),

frais AS (
    SELECT
        natures.aide_id,
        ARRAY_AGG(mapping_.frais) AS "frais"
...
final AS (
    SELECT
        ...
        COALESCE(aides.modifiee_le, aides.mise_a_jour_le) AS "date_maj",
        types.type                                        AS "type",
