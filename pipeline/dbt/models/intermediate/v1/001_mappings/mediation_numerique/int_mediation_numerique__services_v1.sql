...
thematiques AS (
    ...
    FROM {{ ref('stg_mediation_numerique__structures__services') }} AS stg
    INNER JOIN {{ ref('_map_mediation_numerique__thematiques') }} AS map
        ON stg.value = map.service
    GROUP BY stg.structure_id
),
...
frais AS (
    SELECT DISTINCT ON (structure_id)
        ...
        CASE
            ...
            WHEN value = 'Gratuit sous condition' THEN 'gratuit-sous-conditions'
...
modes_mobilisation AS (
    ...
    FROM {{ ref('stg_mediation_numerique__structures__modalites_acces') }} AS stg
    INNER JOIN {{ ref('_map_mediation_numerique__modalites_acces') }} AS map
        ON stg.value = map.modalite_source
    GROUP BY stg.structure_id
),
...
types AS (
    ...
    FROM {{ ref('stg_mediation_numerique__structures__modalites_accompagnement') }} AS stg
...
publics AS (
    ...
    FROM (
        SELECT
            stg.structure_id,
            map.public
        FROM {{ ref('stg_mediation_numerique__structures__publics') }} AS stg
        INNER JOIN {{ ref('_map_mediation_numerique__publics') }} AS map
            ON stg.value = map.public_source
        UNION ALL
        SELECT
            stg.structure_id,
            map.public
        FROM {{ ref('stg_mediation_numerique__structures__prises_en_charge') }} AS stg
        INNER JOIN {{ ref('_map_mediation_numerique__prise_en_charge') }} AS map
            ON stg.value = map.prise_en_charge_source
    ) AS combined
