...
thematiques AS (
    ...
    FROM
        permis_velo,
        UNNEST(permis_velo.liaisons_besoins_mes_aides) AS mes_aides_besoins
    LEFT JOIN mapping_thematiques ON mes_aides_besoins = mapping_thematiques.besoins
...
final AS (
    SELECT
        ...
        (
            SELECT ARRAY_AGG(frais_mapping.frais_di)
            FROM frais_mapping
            WHERE permis_velo.nature && frais_mapping.frais_mes_aides
        )                                                                                                                                       AS "frais",
