...
creches AS (
    SELECT
        ...
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description1'), '^\W*$', ''), '')          AS "description__modalites_tarifaires",
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description2'), '^(\W*|.{1,8})$', ''), '') AS "description__projet",
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description3'), '^\W*$', ''), '')          AS "description__gestionnaire",
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description4'), '^\W*$', ''), '')          AS "description__equipe",
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description5'), '^\W*$', ''), '')          AS "description__conditions_admission",
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'description' ->> 'description6'), '^\W*$', ''), '')          AS "description__modalites_inscription",
        ...
        NULLIF(REGEXP_REPLACE(TRIM(data -> 'serviceAccueil' ->> 'handicap'), '^\W*$', ''), '')           AS "service_accueil__handicap"
    FROM source
),
