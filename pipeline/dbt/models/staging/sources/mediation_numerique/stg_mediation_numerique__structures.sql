...
final AS (
    SELECT
        ...
        CASE
            WHEN source.data ->> 'pivot' != '00000000000000'
                THEN NULLIF(TRIM(source.data ->> 'pivot'), '')
        END                                                                       AS "siret",
        COALESCE(
            source.data -> 'courriels' ->> 0,
            source.data -> 'mediateurs' -> 0 ->> 'email'
        )                                                                         AS "courriel",
        ...
        NULLIF(TRIM(CAST(source.data -> 'adresse' -> 'numero_voie' AS TEXT)), '') AS "adresse__numero_voie",
        NULLIF(TRIM(source.data -> 'adresse' ->> 'repetition'), '')               AS "adresse__repetition",
        CAST(
            ARRAY(
                SELECT d.*
                FROM JSONB_ARRAY_ELEMENTS_TEXT(NULLIF(source.data -> 'dispositif_programmes_nationaux', 'null')) AS d
            ) AS TEXT []
        ...
        NULLIF(
            TRIM(
                CONCAT_WS(
                    ' ',
                    source.data -> 'mediateurs' -> 0 ->> 'prenom',
                    source.data -> 'mediateurs' -> 0 ->> 'nom'
                )
            ),
            ''
        )                                                                         AS "contact_nom_prenom"
    FROM source
